...
sum(is.na(nkill))
sum(is.na(suicide))
sum(is.na(weaptype1_txt))
casualities = length(data1$nkill) + length(data1$nwound) - sum(is.na(data1$nkill)) - sum(is.na(data1$nwound)) 
casualities # so we have 336758 casualities in total

hist(iyear, main = "Number of terrorist activities each year", xlab = "Year", ylab = "count", col = "orange")
cbind(table(iyear))
sort(table(iyear), decreasing = TRUE) # highest number of terrorist activities took
# place in the year 2014

#sort(data.frame(country, country_txt))
x = cbind(table(country_txt))
x = sort(table(country_txt), decreasing = TRUE)
x
# country where highest number of terrorist activities occurred is Iraq

#y = aggregate(data$weaptype1_txt, by = list(kills = nkill), funs(sum(na.rm = TRUE)))
x = data %>% group_by(country_txt) %>% summarise_at(vars(nkill), funs(sum(.,na.rm = TRUE)))
x
max(x$nkill)
which(x$nkill == 78589) 
# iraq has highest number of fatalities with 78589 fatalities

y = data %>% group_by(weaptype1_txt) %>% summarise_at(vars(nkill), funs(sum(.,na.rm = TRUE)))
y
max(y$nkill)
which(y$nkill == 174894)
